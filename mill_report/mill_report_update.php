...
require("mill_calculation.php");
    ...
    $final_extra = monthExtra($lmfd, $lmld) / ($Alluser - 1);

    $member_sub_cost = (millRate($lmfd, $lmld) * userTotalMill($lmfd, $lmld,$user_id)) + $final_extra;

    if ($member_sub_cost < userTotalDeposit($lmfd, $lmld,$user_id)) {
        $member1_cost = (userTotalDeposit($lmfd, $lmld,$user_id) - $member_sub_cost);
        if ($member1_cost == 0) {
        ...
        $member_cost1 = ($member_sub_cost - userTotalDeposit($lmfd, $lmld,$user_id));
        if ($member_cost1 == 0) {
            paidUpdateById($user_id);
            $member_cost = "Paid";
        }else{
    ...
    $mill_sum = userTotalMill($lmfd, $lmld,$user_id);
    $member_sum = userTotalDeposit($lmfd, $lmld,$user_id);
			...
			<div class="col-md-6 col-md-offset-3">
				<table class="table " >
					...
					<tr>
						...
						<td><?php echo millRate($lmfd, $lmld); ?></td>
					</tr>
                    <tr>
